...
export default function BaseballReportApp() {
  ...
  const [gameInfo, setGameInfo] = useState(() => {
    ...
    return {
      title: '25練習試合', away: '相手', home: '八王子', date: '2025/5/4(日)', place: '八王子リトルシニアグラウンド',
      weather: '晴', startHour: '10', startMin: '00', endHour: '12', endMin: '00', homeBatting: true
    };
  });
          ...
          {lineup.map((p:any, idx:number) => {
            ...
            return (
              <div key={idx} className="flex gap-2 mb-2 items-center">
                ...
                >
                  <option value="">選手</option>
                  {playerList.filter((n) => !usedNames.includes(n) || n === p.name).map((n) => (
                    <option key={n}>{n}</option>
                  ))}
                </select>
                <select
